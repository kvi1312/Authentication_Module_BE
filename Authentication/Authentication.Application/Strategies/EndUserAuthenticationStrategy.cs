...
public class EndUserAuthenticationStrategy : IAuthenticationStrategy
{
    ...
    {
        var user = await _unitOfWork.UserRepository.GetByUsernameWithUserTypeAsync(username, UserType.EndUser);

        if (user == null)
        {
            _logger.LogWarning("EndUser not found: {Username}", username);
            return null;
        }

        if (!user.IsActive)
        {
            _logger.LogWarning("EndUser account is inactive: {Username}", username);
            return null;
        }

        if (!_passwordService.VerifyPassword(password, user.PasswordHash))
        {
            _logger.LogWarning("Invalid password for EndUser: {Username}", username);
            return null;
        }

        // Additional EndUser specific validation
        var userWithRoles = await _unitOfWork.UserRepository.GetWithRolesAsync(user.Id);
        if (userWithRoles == null || !userWithRoles.HasUserType(UserType.EndUser))
        {
            _logger.LogWarning("User does not have EndUser role: {Username}", username);
            return null;
        }
    ...
    public Task<Dictionary<string, object>> GetAdditionalClaimsAsync(User user, CancellationToken cancellationToken = default)
    {
    ...
    {
        if (user.HasRole("PremiumEndUser")) return "premium";
        return "standard";
    }
}